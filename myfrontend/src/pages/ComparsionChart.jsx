...
import comparisonPic from '../assets/comparisonchartpic.png'; // make sure this path is correct
...
export default function ComparisonChart() {
  ...
  const handleSubmit = (e) => {
    ...
    const results = allData.filter((item) => {
      ...
      const matchesArea =
        !formData.areaSqFt || item.areaSqFt?.toString().includes(formData.areaSqFt);
      const matchesId =
        !formData.id || item.id?.toString().includes(formData.id);
      const matchesDate =
        !formData.auctionDate || item.auctionDate === formData.auctionDate;

      return matchesBorrower && matchesLocation && matchesBank && matchesArea && matchesId && matchesDate;
    });
      ...
      <div className="w-full flex flex-col md:flex-row gap-6 justify-center items-center">
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-center items-center">
          <h1 className="text-center text-2xl text-[#0B3448] font-semibold sm:font-extrabold mb-3">
            Compare Auction Properties with Ease
          </h1>
          <p className="text-center mb-4 text-[#0B3448] sm:text-lg">
            Our comparison tool allows you to analyze multiple auction properties based on borrower, location, bank, and more.
          </p>
          <img
            src={comparisonPic}
        ...
        {/* Right Column - Form */}
        <div className="w-full md:w-1/2 bg-[#053B50] text-white p-8 rounded-xl shadow-lg">
